...
class Crud extends Component{
    ...
    peticionGet=()=>{
        ...
        firebase.child('damas').on('value',dato=>{
            if(dato.val()!==null){
    ...
    peticionPost=()=>{
        const firebase=app.database().ref();
        firebase.child("damas").push(this.state.form,
          error=>{
    ...
    peticionAddCar=()=>{
      ...
      peticionPut=()=>{
        const firebase=app.database().ref();
        firebase.child(`damas/${this.state.id}`).set(
          this.state.form,
          error=>{
            if(error)console.log(error)
          });
      ...
      peticionDelete=()=>{
        ...
        {
            const firebase=app.database().ref();
        firebase.child(`damas/${this.state.id}`).remove(
          error=>{
            if(error)console.log(error)
          });
        }
             ...
             <h1 className="titulos">Damas</h1>
              ...
              {/* <br/>
                    ...
                    <tbody>
                        {Object.keys(this.state.data).map(i=>{
                            return <tr key={i}>
                                        ...
                                        <td>
                                            <button width="100"><BsFillCartPlusFill onClick={()=>this.seleccionardato(this.state.data[i], i, 'Editar')}/></button>
                                        </td>
                            </tr>
                        })}
                    </tbody>
                ...
                <Modal isOpen={this.state.modalAdd}>
                    <ModalHeader>Agregar dato</ModalHeader>
                    <ModalBody>
                        <div>
